...
#   interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
Hostname=$(hostname)
LANaddrss=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
LANhostname=$(getent hosts $(ip a s) $LANaddrss
extlip=$(curl -s icanhazip.com)
extname=$(getent hosts $extlip awk '{print $2}')

cat <<EOF
Hostname        : $Hostname
LAN Address     : $LANaddrss
