...
numberOfSleeps=19 # how many sleeps to wait for before quitting for inactivity
...
function foundsecret {
 stty sane
     clear
dialog --msgbox "It's the right way to exit the script." 7 60
   exit
}
function reset {
    sleepCount=$numberOfSleeps
        stty sane
      clear
      doCountdown|dialog --gauge "Not to interrupt the sequence." 7 60
}

# Produce the numbers for the countdown
function doCountdown {
while [ $sleepCount -gt 0 ]; do
    echo $((sleepCount * 19 / $numberOfSleeps))
